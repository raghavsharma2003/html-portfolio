...
**WS-R136 (2026-09-05) adds the section's second var, `WHATSAPP_DISPLAY_
PHONE_NUMBER`, still not a credential** — it names a phone number, never a
token — fixing a gap WS-R126 shipped and named honestly as unproven: WS-R126
built a wa.me join link reading `WHATSAPP_PHONE_NUMBER_ID` directly, which
Meta's own Cloud API documents (cited in the table row below, and again in
`api/_room-whatsapp-chat.js`'s own header) confirm is an opaque Graph API
id, not the dialable number a wa.me link needs. This workstream verifies
the real number instead of guessing at it, live, once per process.
...
| `WHATSAPP_DISPLAY_PHONE_NUMBER` | `api/_room-whatsapp-chat.js:whatsappJoinNumber()`, read before any network call, feeding the wa.me join link the share kit's `whatsapp_join` row and the poster's `?channel=whatsapp` variant both build from | optional | E.164 digits only — no leading `+`, no spaces, no dashes (`api/_room-whatsapp-chat.js`'s own `isBareE164Digits`; a value that fails this shape is REFUSED by the link builder, never sanitised down to its digits — WS-R136's own law 4, fixing the gap WS-R126 shipped as NOT PROVEN) | **unset (default)**: the dialable number is read ONCE per process from the Cloud API's own `GET /{phone-number-id}` endpoint (`api/_room-whatsapp.js:fetchPhoneNumberDisplay()`, Meta's own `display_phone_number` field — cited in that function's own header, fetched 2026-09-05 — never `WHATSAPP_PHONE_NUMBER_ID`, which the SAME document names as an opaque Graph API id) and memoised for the life of the process; a provider failure (or absent credentials) records one `provider_whatsapp` incident (skipped for absent credentials — a deploy gap, not a Meta failure) and the number is treated as unknown. **set**: used directly, exactly as configured, no network call is ever made. **either way, if the number ends up unknown or malformed** (unset with a failed fetch, or a configured/fetched value that fails the digits-only shape check — Meta's own two example responses for this same field disagree on shape, one bare digits, one with a leading `+` and separators): `whatsappJoinLink()` returns `null` and BOTH the share kit's `whatsapp_join` row and the poster's `?channel=whatsapp` variant are structurally absent — the studio's Share tab says why, in both locales, rather than ever showing a broken or wrong-number link. |
