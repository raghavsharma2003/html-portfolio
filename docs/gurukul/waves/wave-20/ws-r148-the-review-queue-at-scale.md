# WS-R148: the review queue at scale. A creator with two hundred cards decides them in minutes: keyboard shortcuts on the queue (j/k, s, f, n, u), a bulk selection with one decision for many cards through one statement family that carries every card's own predicate, filters by kind and source, and a count of what is left; both locales, under the layout and accessibility gates. No migration.
...
The review queue is where a creator's judgment enters the product, one
card at a time with three buttons. A real archive mines hundreds. A
creator who gives up at card thirty ships an AI that sounds like the
first thirty.

Laws:
1. Read `api/_review-queue.js` (`decideReviewCard`'s CTE, the kinds, the
   instruction-shaped card), `src/studio/ReviewQueue.tsx`,
   `evals/review-queue/run.mjs`, and WS-R72's picker FIRST.
2. `decide_many`: one op taking up to fifty card ids and one decision,
   executed as the SAME statement family as one card (a `unnest` over the
   ids inside the existing CTE, every predicate per card, never a loop of
   single statements and never a decision the single path would refuse);
   a card another owner holds is skipped by the WHERE and named in the
   result; `never` in bulk writes one rule per card as today; `remove_source`
   in bulk only for instruction-shaped cards, the same WHERE.
3. The panel: keyboard shortcuts with a visible legend, a checkbox per card
   and a select-all-visible, filters by kind (question, claim, delta,
   instruction-shaped, follower-declined) and by source, "N left" from the
   real count; the copy tables carry every string in both locales; the
   layout and accessibility gates get `studio:review-bulk` and
   `studio-hi:review-bulk` fixtures with a hundred cards and the keyboard
   walk reaches the legend and the bulk control.
4. `evals/review-queue` proves the bulk path decides exactly the ids it was
   given and nothing else, the skip-by-name, the never-rules landing per
   card, and a NEGATIVE CONTROL: fifty-one ids refused by name; the door
   battery cases the new op (forged, cross-owner, oversized, redelivered).

## Build

- api/_review-queue.js, api/review-queue.js, src/studio/ReviewQueue.tsx,
  copy tables (append-only), src/studio/layoutFixture.tsx, scripts/check-layout.mjs
  and check-accessibility.mjs targets, evals/review-queue, evals/room-doors.
