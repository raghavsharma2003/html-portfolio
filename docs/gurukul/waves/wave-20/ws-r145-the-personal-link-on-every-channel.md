# WS-R145: the personal link on every channel. A follower on Telegram or WhatsApp gets their referral link and progress with `/friends` (Telegram) and `friends` (WhatsApp), the same hash and the same read the account page uses, in their locale; the referral card on the account page shows the reward's progress; the poster's follower variant is refused by name (a poster is the creator's, never a follower's). No migration.
...
WS-R86's referral link and WS-R130's reward live on the web account page.
Most Indian followers on Telegram or WhatsApp never open it. The growth
loop must be reachable where the follower already is.

Laws:
1. Read `api/_room-referrals.js`, `api/_room-surface.js#roomReferralLink`
   and `roomReferralProgress`, `api/_room-telegram.js`'s command dispatch,
   `api/_room-whatsapp-chat.js`'s command parsing, and WS-R86's leak layer
   FIRST. The link is built by the one existing function; nothing new
   derives a hash.
2. `/friends` on Telegram and `friends` on WhatsApp reply with the
   follower's own link and "N of 3 friends" in their locale, through the
   same session-scoped reads, never a new statement; a paused or unpublished
   Room refuses by name; a follower with referrals off (if a switch exists)
   reads the card.
3. The leak battery's layer 19: two followers on both channels, each reply
   carries only its own hash, byte-checked; the door battery cases both
   commands (redelivery, a forged signature, a phone bound to another Room).
4. `evals/room-telegram`, `evals/room-whatsapp-chat`, `evals/room-referrals`
   extended; the notify-copy table is WS-R144's, so put the two replies'
   strings in `src/room/copy.ts`'s server-readable section the Telegram
   cards already use (read how R84's cards get their locale strings and
   follow it).

## Build

- api/_room-telegram.js, api/_room-whatsapp-chat.js, the copy source the
  cards use (append-only), evals/room-telegram, evals/room-whatsapp-chat,
  evals/room-referrals, evals/room-leak (layer 19), evals/room-doors.
