# WS-R146: rate-limit coverage as a computed property. Every public door (no session, no bearer: the taste, the apply form, the OTP doors, the webhooks, the page doors, the WhatsApp and Telegram inbound doors, the room open) is derived from source and asserted to consume from `vy_public_rate` under a named scope before any read, with the old hand list as the frozen control; the doors the derivation finds without a limit get one; the scopes table becomes the one source the manifest and the transparency page read. No migration.
...
WS-R26 wired the rate table at every public door of its day. Fourteen
waves of doors later, nobody has counted. A public door without a limit
is the one a script finds.

Laws:
1. Read `api/_rate-limit.js` (scopes, `consume`, the daily salt),
   `evals/rate-limit/run.mjs`, `evals/room-doors/run.mjs`'s §0 derivation
   and door classes, and `api/_room-about.js`'s caps section FIRST.
2. Derive the public doors: every derived door whose handler reaches its
   first read without `assertSession`/`userFromToken`/a header secret
   (the battery's own classes tell which), plus every page door. For each,
   a static check that a `consume(` call with a scope literal precedes the
   first database read in the handler's own source (source-scan
   tokenizer), and a dynamic case: the N+1th call in a minute from one
   client key answers 429 with `Retry-After` and writes nothing.
3. The scopes become one exported table (`RATE_SCOPES`, with limit, window
   and the door that owns it); `docs/gurukul/ENV-MANIFEST.md`'s
   `RATE_LIMITS_JSON` section and the transparency page's caps sentence
   read from it (a `// mirror of` marker where a client-side copy exists).
4. Findings fixed in the door's own handler with the pre-fix body as a
   frozen negative control; count of public doors before and after as the
   measurement.

## Build

- api/_rate-limit.js (the table), the doors the derivation names,
  api/_room-about.js, docs/gurukul/ENV-MANIFEST.md (the section's list,
  append-only rows), evals/rate-limit, evals/room-doors (a new class),
  evals/room-about.
