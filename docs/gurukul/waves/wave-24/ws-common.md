# Common brief for every Vyakti workstream, wave twenty-four (2026-09-14)
...
claude/vyakti-cloning-platform-aq05n4. That tip is the merged union of five
...
studio in Hindi, the Room and the landing for any person), and wave twenty-two (WS-R161 to
WS-R171, merged the same day: Meet opens for any person in text first, a
personal AI's Room publishes, the activation guard and real listening clips,
the first five minutes, durable evals, Hindi tier two, the owner's own
continuity in Meet, EmotionOS in the voice, the mobile app proven, data
safety with a schema-mirror gate, and the merge repairs), and wave twenty-three (WS-R172 to
WS-R181, merged 2026-09-13: continuity for a text-ready AI, a person's stage
lines and Room card, a personal AI's Room walked in Chromium, the reply
language policy from the sheet, the register in the voice, the listening test
against the person's own voice, HumanOS drafted from cited claims, the
calibration erasure ordering fixed, the studio's TBT budgets tightened, the
gate honest under load). The main loop (Fable) merges your branch,
...
2. context/STATE.md: the FIRST block ("WAVE TWENTY-THREE MERGED"), then the
   blocks that follow it, oldest last. Codex's stacked "START HERE" blocks below it
...
- Gates: `node scripts/verify-release.mjs` is 25 checks (the schema mirror
  since WS-R170) (typecheck, prompt
  ...
  eval suite (the whole registry, ~440 suites, ~7 min quiet), room leak battery,
  ...
  8946. ONE full gate per machine at a time (`scripts/merge-tools/quiet-gate.sh` waits for load under 8 and every gate port free, then runs it once); ten agents share this machine,
...
- Worktree: `git log --oneline -1` must show the wave-twenty-four base named in
...
- Migrations: idempotent, ONE statement per request (Neon SQL-over-HTTP), no
  ...
  and scripts/relcheck.mjs. Use ONLY the number your brief assigns; **172 is
  the next free number** and the briefs assign from there. The live database
  today carries 015 to 164, 166, 167 and 170 (100, 103, 117, 124, 131, 138,
  157, 165, 168, 169 and 171 unused, 066 to 070 reserved). In your final report list every new SQL statement so the
...
## What the wave-twenty-two merge taught (binding for this wave)

- A workstream that moves a component's user-visible strings into the copy
  registry runs EVERY suite that reads that component's source
  (`grep -rl "<Component>.tsx" evals/`) before it reports; WS-R166 converted
  five components and left ten suites red that only the batch gate found
  (rejected.md, the WS-R171 entries). A suite that scans copy reads the
  component plus the registry's English block for that section, never the
  file alone, and its negative controls mutate the registered value.
- A workstream that adds a statement to a path another suite scans runs that
  suite too (WS-R167's owner-dyad exclusion and evals/room-cohorts).
- The main loop merges under the fast checks and touched suites and runs the
  batch gate; your own full gate runs ONCE at the end when the machine is
  quiet (`cat /proc/loadavg` under 8), otherwise run the whole registry
  `node evals/run.mjs` alone and say so.
- Every fixture answering a widened door carries the new field; every gate
  target keeps its `mounted` selector; a comment never names a follower or
  thread table.

## What the wave-twenty-three merge taught (binding for this wave)

- A browser-side `page.waitForFunction(...)` proves only what the page
  rendered. Any assertion on a Node-side counter that a request fills (a fake
  server's `pending`, `posts`, `reads`) needs its own bounded wait for that
  counter, never a same-tick `assert.equal(x.length, n)`; use
  `boundedWaitMs` from `evals/lib/bounded-wait.mjs` for the bound
  (rejected.md#teacher-sheet-publication-ui-asserted-the-fake-servers-pending-count-on-the-tick-the-button-disabled).
  The same class cost `candidate-activation-ui` and `feedback-dataset-ui` a
  run each under load.
- A fixed 20 to 30 s first-render barrier in a browser suite is load-sensitive;
  scale it with `boundedWaitMs` (WS-R181's core) rather than raising the
  constant. `evals/rehearsal/personal.mjs`'s 20 s wait after sign-in lost twice
  in the pool at load 28 to 40 and passed 63 of 63 alone.
- Every new SQL statement you report is planned on the live database at the
  merge (`EXPLAIN` without `ANALYZE`, parameters bound). Write each statement
  as ONE exported template literal with `$n` placeholders and no runtime
  string interpolation of user values, and list its parameter types in your
  report, so the main loop can bind nil values and plan it in one call. An
  array parameter (`$n::text[]`) must be named as such.
- A follow-up commit that rewrites your own earlier context paragraph is
  legitimate, and the union tool will refuse it ("theirs is not a pure append
  over base"); the main loop resolves it by hand. Prefer a new paragraph that
  supersedes the old one, which merges mechanically.
- Two workstreams appending a numbered section to the same eval file both
  wrote "SECTION 6"; the main loop renumbered one. Name a new section by your
  workstream id (`// SECTION WS-R18x:`), never by the next number.
- A workstream that changes `api/_room-surface.js`'s `roomSay` or `roomSpeak`
  or `src/engine/compiler.ts`'s `CompileInput` will conflict with a sibling
  that does the same; keep such a hunk to the smallest closed block you can,
  and say in your report which lines you touched so the main loop can combine
  both sides.
- The performance gate refuses to judge above load 8 by design; a
  `performance budgets` failure under sibling load is neither a pass nor a
  failure of yours. Run `node scripts/check-performance.mjs` alone once when
  `cat /proc/loadavg` is under 4 if your change touches a measured entry
  point, and report the load it printed.
- Never end a turn waiting on a background monitor (three agents did, and the
  main loop had to message each one to report). Gate in the foreground with a
  timeout, commit, `git status` clean, full report in the same turn.
