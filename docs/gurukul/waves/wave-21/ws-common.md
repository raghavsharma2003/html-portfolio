# Common brief for every Vyakti workstream, wave twenty-one (2026-09-13)
...
claude/vyakti-cloning-platform-aq05n4. That tip is the merged union of three
...
migrations 137 to 162), and the main loop's repairs of 2026-09-13 (the tree
deploys again, the gate is green). The main loop (Fable) merges your branch,
...
2. context/STATE.md: the FIRST block ("2026-09-13 base") and the wave-era
   START HERE that follows it. Codex's stacked "START HERE" blocks below it
...
- Worktree: `git log --oneline -1` must show the wave-twenty-one base named in
...
- Migrations: idempotent, ONE statement per request (Neon SQL-over-HTTP), no
  ...
  and scripts/relcheck.mjs. Use ONLY the number your brief assigns; **163 is
  the next free number** and the briefs assign from there. The live database
  today carries the wave era through 136; 137 to 162 are being applied by
  the main loop. In your final report list every new SQL statement so the
...
- Front end: React + TypeScript; tokens from src/studio/design/tokens.css or
  ...
  there adds its Hindi through the same registry pattern it names). A new
