...
## Vercel access and Foundry correction, 2026-09-14

The owner supplied working Vercel access; earlier access-blocked statements above are historical. Existing variables now include Preview (22 names) and Development where their protection type permits it (5 names). Fourteen Azure provider settings and two scoped API-key rows were subsequently applied and independently read back. Existing Azure Terra serves dialogue and mini serves extraction under a $1 pilot text cap. No deployment or inference was performed by this configuration step. Additional runtime key requirements remain under caller audit. The credential is OS-protected outside Git; do not paste it into context.

For the later gated cutover, the official Vercel Terraform client implements production-branch update as PATCH /v9/projects/{id}/branch with body {"branch":"main"}. Its Git-link endpoint is POST /v9/projects/{id}/link. No branch or link cutover has been performed. Source: https://github.com/vercel/terraform-provider-vercel/blob/main/client/project.go (UpdateProductionBranch and LinkGitRepoToProject).
