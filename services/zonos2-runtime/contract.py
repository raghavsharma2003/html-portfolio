"""Signed, fail-closed contract for the isolated ZONOS2 evaluation lane."""
...
def decode_secret(raw: str) -> bytes:
    try:
        ...
        raise RuntimeError("zonos2_hmac_secret_invalid") from exc
    if len(value) < 32:
        raise RuntimeError("zonos2_hmac_secret_required")
    return value
...
def reference(payload: dict[str, Any]) -> Reference:
    ...
    if duration_ms < 6_000 or duration_ms > 12_000:
...
def validate_payload(payload: Any) -> dict[str, Any]:
    ...
    if language_id not in DISCLOSURES:
        raise ServiceError("zonos2_language_unsupported", 422)
    ...
    if language_id == "hi" and script_mode != "devanagari":
        raise ServiceError("hindi_devanagari_required", 422)
    if language_id == "hinglish" and script_mode != "mixed":
        raise ServiceError("hinglish_mixed_script_required", 422)
    ...
    if payload.get("clone_mode") != "accurate_speaker_embedding":
    ...
    return {
        ...
        "clone_mode": "accurate_speaker_embedding",
        "seed": seed,
        "reference": ref,
