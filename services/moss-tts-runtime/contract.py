"""Signed, fail-closed contract for the isolated MOSS-TTS v1.5 lane."""
...
def decode_secret(raw: str) -> bytes:
    try:
        ...
        raise RuntimeError("moss_tts_hmac_secret_invalid") from exc
    if len(value) < 32:
        raise RuntimeError("moss_tts_hmac_secret_required")
    return value
...
def validate_payload(payload: Any) -> dict[str, Any]:
    ...
    if language_id not in DISCLOSURES:
        raise ServiceError("moss_tts_language_unsupported", 422)
    text = _text(payload.get("text"), "text_invalid", MAX_TEXT_CHARS)
    if not text.startswith(DISCLOSURES[language_id] + " "):
        raise ServiceError("localized_disclosure_required", 422)
    script_mode = _script_mode(text)
    ...
    if payload.get("clone_mode") != "zero_shot_reference":
        raise ServiceError("moss_tts_clone_mode_invalid", 422)
    reference_text = payload.get("reference_text")
    reference_text_sha256 = payload.get("reference_text_sha256")
    if reference_text is None and reference_text_sha256 is not None:
        raise ServiceError("reference_text_binding_invalid", 422)
    if reference_text is not None:
        reference_text = _text(reference_text, "reference_text_invalid", MAX_REFERENCE_TEXT_CHARS)
        reference_text_sha256 = str(reference_text_sha256 or "")
    ...
    if evaluation_scope == "verified_owner_identity":
        ...
        if payload.get("training_allowed") is not True or payload.get("identity_claim_allowed") is not True:
            raise ServiceError("owner_identity_use_binding_required", 422)
    ...
    return {
        ...
        "language_label": LANGUAGE_LABELS[language_id],
        "script_mode": script_mode,
        "text": text,
        "text_sha256": sha256(text.encode()),
        "disclosure": DISCLOSURES[language_id],
        "clone_mode": "zero_shot_reference",
