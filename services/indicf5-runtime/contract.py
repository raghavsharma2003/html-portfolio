"""Stdlib-only signed contract for the isolated IndicF5 evaluation runtime."""
...
from pronunciation_normalizer import (
    CONTRACT as PRONUNCIATION_NORMALIZATION_CONTRACT,
    DOMAIN as PRONUNCIATION_DOMAIN,
    LOCALE as PRONUNCIATION_LOCALE,
...
MAX_TEXT_CHARS = MAX_SOURCE_CODEPOINTS
...
def decode_secret(raw: str) -> bytes:
    try:
        ...
        raise RuntimeError("indicf5_hmac_secret_invalid") from exc
    if len(value) < 32:
        raise RuntimeError("indicf5_hmac_secret_required")
    return value
...
def script_mode(value: str) -> str:
    devanagari = any("\u0900" <= char <= "\u097f" for char in value)
    latin = any("A" <= char <= "Z" or "a" <= char <= "z" for char in value)
    if devanagari:
...
def validate_payload(payload: Any) -> dict[str, Any]:
    ...
    if str(payload.get("language_id", "")) != "hi":
        raise ServiceError("indicf5_hindi_only", 422)
    text = _text(payload.get("text"), "text_invalid", MAX_TEXT_CHARS)
    if not text.startswith(f"{DISCLOSURE} ") or script_mode(text) not in {"devanagari", "mixed"}:
        raise ServiceError("hindi_disclosure_required", 422)
    text_hash = str(payload.get("text_sha256", ""))
    if not SHA_RE.fullmatch(text_hash) or sha256(text.encode("utf-8")) != text_hash:
        raise ServiceError("text_digest_invalid", 422)
    pronunciation_normalization = payload.get("pronunciation_normalization")
    expected_pronunciation_normalization = {
        "contract": PRONUNCIATION_NORMALIZATION_CONTRACT,
        "domain": PRONUNCIATION_DOMAIN,
        "locale": PRONUNCIATION_LOCALE,
        "mode": "required",
    }
    if pronunciation_normalization != expected_pronunciation_normalization:
        raise ServiceError("pronunciation_normalization_contract_invalid", 422)
    ...
    if not SHA_RE.fullmatch(ref_text_hash) or sha256(ref_text.encode("utf-8")) != ref_text_hash:
    ...
    return {
        ...
        "text_language_mode": script_mode(text),
        "pronunciation_normalization": dict(expected_pronunciation_normalization),
        "reference_text": ref_text,
        "reference_text_sha256": ref_text_hash,
        "reference_language_mode": script_mode(ref_text),
