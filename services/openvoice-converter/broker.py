"""Public signed admission for the private OpenVoice converter GPU."""
...
def _decode_secret() -> bytes:
    raw = os.getenv("OPENVOICE_CONVERTER_HMAC_SECRET", "")
    try:
        value = (
            bytes.fromhex(raw)
...
def _runtime_origin() -> str:
    raw = os.getenv("OPENVOICE_CONVERTER_RUNTIME_ORIGIN", "")
    parsed = urlsplit(raw)
    if (
        parsed.scheme != "https"
        ...
        raise RuntimeError("openvoice_converter_runtime_origin_invalid")
...
def _signature(secret: bytes, *values: str) -> str:
    digest = hmac.new(secret, "\n".join(values).encode(), hashlib.sha256).digest()
    return base64.urlsafe_b64encode(digest).rstrip(b"=").decode()


def _signed_response(
    request: Request, body: bytes, status: int, content_type: str = "application/json"
) -> Response:
    nonce = request.headers.get("x-vyakti-nonce", "")
    response = Response(status_code=status, content=body, media_type=content_type)
...
async def _admit(request: Request) -> tuple[bytes, str]:
    ...
    if declared and (
        not declared.isdigit() or int(declared) > MAX_REQUEST_BYTES
    ):
    ...
    if request.headers.get("x-vyakti-protocol") != PROTOCOL or not re.fullmatch(
        r"[A-Za-z0-9_-]{20,64}", nonce
    ):
    ...
    if (
        abs(time.time() - issued_at) > MAX_CLOCK_SKEW_SECONDS
        or body_hash != _sha(body)
...
async def _runtime_is_ready() -> bool:
    try:
        response = await app.state.wake_client.get(
            f"{app.state.runtime_origin}/healthz"
        )
...
async def lifespan(application: FastAPI):
    application.state.secret = _decode_secret()
...
app = FastAPI(
    title="Vyakti OpenVoice Converter Admission",
    docs_url=None,
    redoc_url=None,
    openapi_url=None,
    lifespan=lifespan,
...
async def health() -> JSONResponse:
    return JSONResponse(
        ...
        content={"ready": bool(getattr(app.state, "ready", False))},
    )
...
async def convert(request: Request) -> Response:
    try:
        ...
        if not await _runtime_is_ready():
            raise BrokerError("openvoice_converter_warming", 503)
        ...
        if not hmac.compare_digest(
            expected, upstream.headers.get("x-vyakti-response-signature", "")
        ):
        ...
        return _signed_error(request, "openvoice_converter_unreachable", 503)
