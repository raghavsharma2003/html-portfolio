...
type Props={token:string;ownerUserId:string;replicaId:string;expectedSourceId:string;onAuthError:(error:unknown)=>void};
...
export default function ComparisonReferenceReview({token,ownerUserId,replicaId,expectedSourceId,onAuthError}:Props){
 ...
 const option=options?.options.find(o=>o.artifact_id===selected&&o.source_id===expectedSourceId);
 ...
 async function audition(){if(!saved||saved.withdrawing||!reference?.can_audition)return;await run(async()=>{const ticket=generation.current;
  ...
  setReference(value);if(!value.can_audition||value.source_id!==expectedSourceId)throw Error("comparison_changed");
  const url=URL.createObjectURL(blob);audioRef.current=url;setAudio(url);
 });}
 async function confirm(){if(!saved||saved.withdrawing||!confirmed||!reference?.can_confirm||!reference.snapshot_hash||reference.source_id!==expectedSourceId)return;
  await run(async()=>{const ticket=generation.current;const value=await writeComparisonReference(token,replicaId,saved.id,{op:"confirm",expected_snapshot_hash:reference.snapshot_hash!,confirm_this_is_my_voice:true},controller.current.signal);
   if(!current(ticket))return;setReference(value);clearAudio();setMessage(value.state==="selected"?"Saved for private comparison.":"Check the saved status before continuing.");},true);
 }
 async function withdraw(){if(!saved)return;await run(async()=>{const ticket=generation.current;
  ...
  setReference(value);if(value.state!=="revoked")throw Error("withdraw_unconfirmed");persist({...saved,withdrawing:false});
 },true);}
 ...
 return <section className="cvj-comparison-reference" aria-labelledby="comparison-reference-title">
  ...
  {!saved&&options?.state==="unavailable"&&!options.next_cursor?<p>We do not have compatible prepared evidence for this recording yet. No new permission is needed here.</p>:null}
  ...
  {!saved&&options?.state==="available"?<>
   <label>Prepared version<select value={selected} disabled={busy} onChange={e=>{setSelected(e.target.value);setChecks({});clearAudio();}}>
    <option value="">Choose a version</option>{options.options.filter(o=>o.source_id===expectedSourceId).map((o,i)=><option key={o.artifact_id} value={o.artifact_id}>Version {i+1}, {new Date(o.source_created_at).toLocaleDateString()}{o.duration_ms?`, ${Math.round(o.duration_ms/1000)} seconds`:""}</option>)}
   </select></label>
  ...
  {saved?<>
   ...
   {!saved.withdrawing&&reference?.can_audition&&reference.source_id===expectedSourceId?<button className="cvj-primary" type="button" disabled={busy} onClick={()=>void audition()}>Open private recording</button>:null}
