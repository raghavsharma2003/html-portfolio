...
export async function replicaRequest<T>(token: string, path: string, init?: RequestInit): Promise<T> {
  ...
  try {
    ...
    // Body cancellation must not turn a successful HTTP status into empty data.
...
export async function createReplica(token: string, displayName: string, creationIntentId: string): Promise<Replica> {
  const data = await replicaRequest<{ replica: Replica; creation_intent_id: string; replayed: boolean }>(token, "/api/replica", {
    method: "POST",
    body: JSON.stringify({ op: "create", display_name: displayName, creation_intent_id: creationIntentId }),
  });
  return data.replica;
}
