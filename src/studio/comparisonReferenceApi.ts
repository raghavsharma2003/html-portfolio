...
export type ComparisonOption={purpose?:"ordinary"|"comparison_reference";preparation_id?:string;artifact_id:string;source_id:string;source_created_at:string;duration_ms:number|null;mime:"audio/wav"|"audio/x-wav";snapshot_hash:string};
...
export async function getComparisonOptions(token:string,replicaId:string,signal:AbortSignal,cursor:string|null=null):Promise<ComparisonOptions>{
 ...
 if(d?.replica_id!==replicaId||d.statement_set!=="private-comparison-reference/v1"||d.capture_ready!==false||
  ...
  d.state!==(d.options.length?"available":"unavailable")||d.options.some(o=>!comparisonId(o.artifact_id)||!comparisonId(o.source_id)||
    (o.purpose!==undefined&&!["ordinary","comparison_reference"].includes(o.purpose))||(o.purpose==="comparison_reference"&&!comparisonId(o.preparation_id))||!date(o.source_created_at)||!hash(o.snapshot_hash)||!["audio/wav","audio/x-wav"].includes(o.mime)||
...
export function validateComparisonReference(d:ComparisonReference,replicaId:string,referenceId:string):ComparisonReference{
 if((d?.purpose!==undefined&&d.purpose!=="comparison_reference")||(d?.purpose==="comparison_reference"&&(!comparisonId(d.preparation_id)||!comparisonId(d.source_id)))||d?.replica_id!==replicaId||d.reference_id!==referenceId||!["review","selected","revoked","expired"].includes(d.state)||!date(d.created_at)||
