...
type CalibrationProps = { token: string; replicaId: string; onAuthError: (cause: unknown) => void };

export default function CalibrationStudio(props: CalibrationProps) {
  return <ScopedCalibrationStudio key={`${props.replicaId}:${props.token}`} {...props} />;
}

function ScopedCalibrationStudio({ token, replicaId, onAuthError }: CalibrationProps) {
  const mounted = useRef(false), readRevision = useRef(0), mutation = useRef(false);
  useEffect(() => {
  ...
  const load = useCallback(async () => {
    const revision = ++readRevision.current, current = () => mounted.current && revision === readRevision.current;
    setLoading(true);
    setError("");
    try {
      const next = await readCalibration(token, replicaId);
      if (!current()) return;
      if (next?.replica_id !== replicaId) throw new Error("The reviewed calibration could not be confirmed. Retry.");
      setStatus(next);
    } catch (cause) {
      if (!current()) return;
  ...
  async function choose(choice: CalibrationChoice) {
    if (!current || !mounted.current || mutation.current || status?.replica_id !== replicaId) return;
    ...
    try {
      ...
      if (mounted.current) { mutation.current = false; setBusy(false); }
    }
  }
