...
export default function StudioApp({
  ...
  initialSession?: StudioSession | null;
  ...
  const [authChecked, setAuthChecked] = useState(sessionAlreadyRestored);
  ...
  useEffect(() => {
    let live = true;
    if (sessionAlreadyRestored) {
      // Keep StrictMode's discarded first effect from starting a duplicate
      // workspace read. The entry controller has already restored exactly
      // this session; it must never be restored again after an in-app sign-out.
      Promise.resolve().then(() => {
        if (live && initialSession) void loadReplicas(initialSession);
      });
      return () => { live = false; };
    }
    ...
    return () => { live = false; };
  }, [initialSession, loadReplicas, sessionAlreadyRestored]);
