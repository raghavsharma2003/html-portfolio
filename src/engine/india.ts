...
export async function writeKin(q: QueryFn, input: WriteKinInput): Promise<number> {
  if (!input.citations || input.citations.length < 1) {
    throw new Error(`vy_kin requires >=1 citation (${input.name} for ${input.person_id})`);
  }
  const rows = await q(
    `insert into vy_kin (person_id, name, relation, fictive, address_term, citations)
     values ($1,$2,$3,$4,$5,$6)
     on conflict (person_id, lower(name)) do update set
       ...
       updated_at = now()
     returning id`,
    [input.person_id, input.name, input.relation, input.fictive ?? false, input.address_term ?? "", input.citations],
  );
...
export async function recordRitualOccurrence(
  ...
  await q(
    `insert into vy_ritual (person_id, key, last_at, count, cold_last, citations)
     values ($1,$2, now(), 1, $3, array[$4]::bigint[])
    ...
    [personId, key, coldReception, citation],
  );
}
...
export async function writeCurrencyUse(
  ...
  await q(
    ...
    [personId, topic, kind, citation === null ? [] : [citation]],
  );
}
...
export async function writeIndiaProfile(q: QueryFn, input: WriteIndiaProfileInput): Promise<void> {
  ...
  const existing = await q(`select sensitive_consent from vy_india_profile where person_id = $1`, [input.person_id]);
  const mergedConsent = {
  ...
  await q(
    `insert into vy_india_profile
       (person_id, mother_tongue, home_region, religion, family_structure, dietary, sensitive_consent, updated_at)
