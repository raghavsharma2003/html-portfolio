...
export default function StudioApp() {
  ...
  const createIntentRef = useRef<{ name: string; id: string } | null>(null);
  const createLockedRef = useRef(false);

  async function handleCreate(name: string) {
    if (!session || createLockedRef.current) return;
    createLockedRef.current = true;
    if (!createIntentRef.current || createIntentRef.current.name !== name) {
      createIntentRef.current = { name, id: crypto.randomUUID() };
    }
    setCreating(true);
    setError(null);
    try {
      const fresh = await refreshForRequest(session);
      const replica = await createReplica(fresh.accessToken, name, inviteCode || undefined, createIntentRef.current.id);
