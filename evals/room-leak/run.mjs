...
// ═════════════════════════════════════════════════════════════════════════
// LAYER 17 (WS-R133, referral-reward hardening, law 4). The full world seeds
...
// ═════════════════════════════════════════════════════════════════════════
console.log("\n── layer 17: referral credits and rewards (WS-R133) — isolation, and the race ──");
{
  const w = await runFullWorld(REPO);
  const { db, deps, room, sessionOf, referralSeed } = w;
  const { roomReferralProgress, roomExport, REFERRAL_REWARD_FRIEND_THRESHOLD } = room;

  ok("layer 17: the world seeded at least one referrer with credited friends (not vacuous)",
    referralSeed.length > 0, referralSeed.length);
  ok("layer 17: at least one seeded referrer crosses the reward threshold and has a granted reward",
  ...
  for (const seed of referralSeed) {
    ...
    checked++;
    ok(`layer 17: referrer ${seed.referrerIdx} (Room ${seed.roomIdx})'s own progress reads ONLY their own credited friends (${seed.friendFollowerIds.length})`,
    ...
    if (seed.rewardGranted) {
      checked++;
      ok(`layer 17: referrer ${seed.referrerIdx}'s own progress read carries their OWN granted reward`, progress.reward !== null);
    }
    ...
    for (const other of referralSeed) {
      ...
      checked++;
      ok(`layer 17 BYTE-CHECK: referrer ${other.referrerIdx}'s own progress read never carries referrer ${seed.referrerIdx}'s follower id`,
    ...
    for (const fIdx of seed.friendIdxs) {
      ...
      checked++;
      ok(`layer 17: referred friend ${fIdx}'s own export shows exactly one "I was referred" credit, never the referrer's identity`,
        friendExport.tables?.vy_room_referral_credit?.count === 1 &&
          !JSON.stringify(friendExport).includes(seed.referrerFollowerId));
    }
  }
  ok(`layer 17: ${checked} referral read-isolation checks ran (not vacuous)`, checked > 0, checked);
  ...
  // (a) FRIEND_2's webhook processed BEFORE FRIEND_3's.
  {
    ...
    ok("layer 17 RACE (order A, FRIEND_2 then FRIEND_3): exactly ONE reward is granted",
      grants.length === 1, JSON.stringify([r2, r3]));
    ok("layer 17 RACE (order A): exactly one reward row exists for this referrer/room/year",
  ...
  // (b) the OPPOSITE order — FRIEND_3's webhook processed BEFORE FRIEND_2's.
  {
    ...
    const r2 = await maybeGrantReferralReward(db2, ...grantArgs(ROOM, FRIEND_2, "race-evt-2"));
    const grants = [r2, r3].filter((r) => r !== null);
    ok("layer 17 RACE (order B, FRIEND_3 then FRIEND_2): exactly ONE reward is granted",
      grants.length === 1, JSON.stringify([r2, r3]));
    ok("layer 17 RACE (order B): exactly one reward row exists for this referrer/room/year",
      state.rewards.filter((r) => r.referrer_follower_id === REFERRER_FOLLOWER).length === 1);
  }
  ...
  //     own microtask yield is what lets them genuinely interleave.
  {
    ...
    const grants = [r2, r3].filter((r) => r !== null);
    ok("layer 17 RACE (concurrent, Promise.all): exactly ONE reward is granted, never zero, never two",
      grants.length === 1, JSON.stringify([r2, r3]));
    ok("layer 17 RACE (concurrent): exactly one reward row exists for this referrer/room/year",
      state.rewards.filter((r) => r.referrer_follower_id === REFERRER_FOLLOWER).length === 1);
    ok("layer 17 RACE (concurrent): the referrer's OWN subscription was extended exactly once (the loser's write never re-extended it)",
