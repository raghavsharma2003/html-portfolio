...
console.log("\n── layer 19: RelationOS in the Room (vy_rel_state/vy_rel_event, two followers, one dyad key each) ──");
{
  ...
  const dbStage19 = async (sql, params = []) => {
    ...
    if (sql.includes("select subject_person_id from vy_replica")) {
      const agentId = String(params[0]);
      return REPLICA_A_19.agent_id === agentId ? [{ subject_person_id: REPLICA_A_19.subject_person_id }] : [];
    }
    if (sql.includes("select trust, rupture_open from vy_rel_state")) {
      const [personId, agentId] = params.map(String);
      return ownerDyadRow19.person_id === personId && ownerDyadRow19.agent_id === agentId ? [ownerDyadRow19] : [];
    }
