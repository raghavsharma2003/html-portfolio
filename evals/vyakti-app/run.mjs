  ...
  if (!existsSync(distStudioFixture)) {
    ...
    if (!browser) {
      ...
      try {
        ...
        // WS-R166 (wave twenty-two) moved the shell's strings into the copy registry; the
        // two upload-scene checks read the component plus the English table.
        const studioCopySrc = readFileSync(join(ROOT, "src/studio/copy.ts"), "utf8");
        ...
        ok("CloneExperience.tsx: the upload scene's heading text is exactly \"Securing your recording.\" for every phase but failed", (/upload\.phase === "failed" \? (?:"Upload paused\."|copy\.upload\.pausedHeading) : (?:"Securing your recording\."|copy\.upload\.securingHeading)/.test(cloneExperienceSrc) && /securingHeading: "Securing your recording\."/.test(studioCopySrc)));
        ok("CloneExperience.tsx: the upload scene shows live progress (\"Uploading N%\") during the upload phase specifically", (/upload\.phase === "upload" \? (?:`Uploading \$\{upload\.progress\}%`|copy\.upload\.uploadingPercentTemplate\.replace\("\{n\}", String\(upload\.progress\)\))/.test(cloneExperienceSrc) && /uploadingPercentTemplate: "Uploading \{n\}%"/.test(studioCopySrc)));
