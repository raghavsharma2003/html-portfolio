...
try{browser=await chromium.launch({headless:true});const page=await browser.newPage();await page.route('https://synthetic.blob.core.windows.net/**',r=>r.fulfill({status:201,body:''}));
 ...
 prepared=true;await page.getByRole('button',{name:'Check recording status',exact:true}).click();await page.getByText('Your recording is prepared. Choose it below to review.').waitFor();assert.deepEqual(await page.evaluate(()=>window.preparationFixture.prepared),[sid]);receipt.checks.push('prepared notifies parent without choosing reference');
