...
function saveState(state) {
  ...
  writeFileSync(join(OUT, "state.json"), JSON.stringify(state, null, 1));
}
...
async function main() {
  ...
  const { pool, evidence, promptFieldFound } = buildPool();
  if (promptFieldFound) {
    console.error(
  ...
  if (!RUN) {
    planReport(pool, evidence);
    process.exit(0);
  }

  // Repeat-sample wraparound if --limit exceeds the 288-unit pool (see header).
  let units = [];
  ...
  console.log(
    `── WS-CANDGEN live run — terra, ${units.length} unit(s) requested, ${units.length - todo.length} already ` +
      `done (resumed), ${todo.length} to call ──\n`,
  );
  ...
  mkdirSync(OUT, { recursive: true });
  writeFileSync(join(OUT, "candidate-transcript.json"), JSON.stringify(transcript, null, 1));
  ...
  const byteIdentical = 0; // structural finding — see file header; never derived from this run's data
  const reconstructed = allDone.length;
  ...
  console.log(`\nWrote evals/candidate/out/candidate-transcript.json (${transcript.length} turns)`);
  console.log(`Wrote evals/candidate/out/manifest.json`);
