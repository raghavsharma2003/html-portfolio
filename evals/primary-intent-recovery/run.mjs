...
try{
 ...
 for(const change of ['scope','deleted','consent','unmount'])await check(`pending read ${change}: no new request`,async()=>{await open('pending-'+change);await action().click();await page.waitForFunction(()=>window.recoveryProbe.reads===1);await page.evaluate(change=>{if(change==='unmount')window.recoveryProbe.unmount();else window.recoveryProbe.change(change);},change);await page.evaluate(()=>window.recoveryProbe.resolve());await page.waitForTimeout(80);assert((await calls()).every(c=>c.buildIntentId==='50000000-0000-4000-8000-000000000005'));});
