...
// `api/consolidate-sweep.js`'s OWN real `authorized()` source (a genuine
// query/body secret fallback this workstream found and left OUT of scope,
// `rejected.md#ws-r89-consolidate-sweep-secret-in-query-or-body-found-out-
// of-scope`), the extraction correctly flags it as reading `req.query`/
// `req.body` for the secret.
{
  const consolidateSrc = readFileSync(join(API, "consolidate-sweep.js"), "utf8");
  const fnMatch = consolidateSrc.match(/function \w*[Aa]uthoriz\w*\([^)]*\)[^{]*\{[\s\S]*?\n\}/);
  const fnBody = fnMatch ? fnMatch[0] : "";
  const wouldPass = Boolean(fnMatch) && /req\??\.headers/.test(fnBody) && !/req\??\.query/.test(fnBody) && !/req\??\.body/.test(fnBody);
  ok(
    "NEGATIVE CONTROL: this class's own extraction correctly flags api/consolidate-sweep.js's REAL authorized() as reading req.query/req.body for the secret (the reason it stays out of scope rather than silently passing)",
