...
section("route identity boundary");
{
  ...
  check("the HMAC secret never crosses to the client",
    ...
    !readFileSync(join(ROOT, "src/creatorStudio/voicePanelApi.ts"), "utf8").includes("HMAC") &&
    !readFileSync(join(ROOT, "src/creatorStudio/VoicePreviewPanel.tsx"), "utf8").includes("AZURE_OPEN_VOICE_ORIGIN"));
  check("the client never calls the broker directly",
    !/azurecontainerapps\.io/.test(readFileSync(join(ROOT, "src/creatorStudio/voicePanelApi.ts"), "utf8")));
}
...
{
  const client = readFileSync(join(ROOT, "src/creatorStudio/VoicePreviewPanel.tsx"), "utf8");
  const clientApi = readFileSync(join(ROOT, "src/creatorStudio/voicePanelApi.ts"), "utf8");
  // WS-R71: VoicePreviewPanel.tsx's own literal strings moved into
  // src/creatorStudio/copy.ts; the two English-wording checks below now read this
  ...
  const copySource = readFileSync(join(ROOT, "src/creatorStudio/copy.ts"), "utf8");
