...
function makeDb(state) {
  ...
  const db = async (sql, params = []) => {
    ...
    if (has("with candidate as") && has("insert into vy_payment_event")) {
      ...
      // WS-R130 (migration 133): `su.follower_id` joined the real SELECT
      // list alongside `su.person_id` - `maybeGrantReferralReward`'s own
      // caller in `applyWebhook` needs it, `result.person_id`'s own
      // precedent one field over.
      return [{ event_id: event.event_id, subscription_id: subId, state: sub ? sub.state : null, person_id: sub ? sub.person_id : null, follower_id: sub ? sub.follower_id : null, tier }];
    }
...
// ═════════════════════════════════════════════════════════════════════════
console.log("\n§17 WS-R130 (migration 133) — the referral reward, granted from THIS webhook");
// ═════════════════════════════════════════════════════════════════════════
{
  const state = freshState();
  state.prices.push({ room_id: ROOM, owner_user_id: OWNER, follower_price_inr: 399, currency: "INR", platform_take_bp: 2500 });
