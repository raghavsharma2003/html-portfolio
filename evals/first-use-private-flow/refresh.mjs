...
// WS-R181. `context/rejected.md#first-use-refresh-suite-races-under-load`:
// this suite's own bounded barrier was already widened once, by hand, from
// 10s to 30s, after measuring it expire under the pooled registry -- the
// exact ad hoc version of what boundedWaitMs now does for every suite in
// this shape, scaled to how loaded the machine actually is right now rather
// than a second hand-picked constant.
...
try{
 ...
 const waitFor=async(label,predicate)=>{const end=Date.now()+boundedWaitMs(30000);/* bounded, not fixed: 10 s expired under the pooled registry (3 workers, 2 browsers) while the suite passed alone, 2026-09-13; now WS-R181's own load-scaled base rather than a second hand-picked constant */while(!predicate()&&Date.now()<end)await new Promise(r=>setTimeout(r,10));assert(predicate(),`${label}: bounded HTTP barrier; state=${httpState()}`);};
 ...
 for(const width of [390,1440])for(const method of ['create','list','select'])for(const variant of ['old','current'])await check(`${variant} ${width}: pending ${method} settles after actual readiness refresh`,async()=>{
  for(const outcome of ['success','error']){
   ...
   await waitFor('operation request queued',()=>pending.some(p=>p.kind==='operation'));
