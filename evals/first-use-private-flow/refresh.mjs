...
try{
 ...
 const waitFor=async(label,predicate)=>{const end=Date.now()+30000;/* bounded, not fixed: 10 s expired under the pooled registry (3 workers, 2 browsers) while the suite passed alone, 2026-09-13 */while(!predicate()&&Date.now()<end)await new Promise(r=>setTimeout(r,10));assert(predicate(),`${label}: bounded HTTP barrier; state=${httpState()}`);};
