...
// WS-R181. This suite's own bounded HTTP barrier and its Playwright default
// action timeout were both fixed constants tuned on a quiet machine; scale
// them by how loaded THIS machine is right now (see evals/lib/bounded-wait.mjs's
// header and context/rejected.md#first-use-refresh-suite-races-under-load,
// the sibling suite that first found this exact shape).
import {boundedWaitMs} from '../lib/bounded-wait.mjs';
const root=fileURLToPath(new URL('../../',import.meta.url));
const artifact=join(root,'scratchpad/first-use-activity-abort',String(Date.now()));mkdirSync(artifact,{recursive:true});
...
const hashes=Object.fromEntries(['src/studio/replicaApi.ts','evals/first-use-private-flow/fixtures/old-replicaApi.ts.txt','src/studio/StudioApp.tsx','src/studio/CloneExperience.tsx','src/studio/ContextLockerPanel.tsx','src/studio/PrivateTextRehearsal.tsx','evals/first-use-private-flow/fixtures/old-studio.tsx.txt','evals/first-use-private-flow/fixtures/old-experience.tsx.txt'].map(p=>[p,createHash('sha256').update(readFileSync(join(root,p))).digest('hex')]));
...
try{
 ...
 server=createServer(async(req,res)=>{
  try{
   ...
   if(url.pathname.startsWith('/api/')){
    ...
    if(url.pathname==='/api/replica-activity'){const value={replica_id:rid,generated_at:'2026-09-07T00:00:00Z',jobs:[],lanes:[],in_flight:false,next_poll_ms:null};if(scenario.includes('overlap')&&req.headers.authorization===`Bearer ${TOKEN}`){const json=JSON.stringify(value);res.writeHead(200,{'content-type':'application/json','cache-control':'no-store'});res.write(json.slice(0,12));pending.push({kind:'activity-body',send:()=>{if(!res.destroyed)res.end(json.slice(12));}});return;}return send(200,value);}
 ...
 browser=await launchSuiteBrowser("first-use-activity-abort-ui");
 let page;
 const count=(path,op)=>requests.filter(r=>r.path===path&&r.op===op).length;
 const waitFor=async(predicate)=>{const end=Date.now()+boundedWaitMs(10000);while(!predicate()&&Date.now()<end)await new Promise(r=>setTimeout(r,10));assert(predicate(),'bounded HTTP barrier');};
 ...
 for(const width of [390,1440])for(const variant of ['old','current'])await check(`${variant} ${width}: native Activity body abort during token refresh`,async()=>{
  await open(variant,width);await page.getByRole('button',{name:'Open your clones',exact:true}).click();await page.getByRole('button',{name:'Create another clone',exact:true}).click();scenario='create-overlap-success';await agree();await waitFor(()=>pending.some(p=>p.kind==='operation'));
 ...
 assert.deepEqual(errors,[]);assert.deepEqual(surprises,[]);writeFileSync(join(artifact,'result.json'),JSON.stringify({passed:checks.length,checks,focus,hashes,errors,surprises,scope:'Actual Studio and ActivityPanel, real localhost partial200 body aborted by token-effect cleanup; synthetic account refresh, no real auth/DB/model',expectedOldCrashes,completed_at:new Date().toISOString()},null,2));console.log(JSON.stringify({passed:checks.length,artifact}));
