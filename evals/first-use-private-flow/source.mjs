...
for(const mounted of [false,true])for(const mutant of [false,true]){
 ...
 compile(code+';globalThis.run=continueAgreement;',scope);const task=scope.run();release(props.selected);await task;assert.deepEqual(writes,mounted||mutant?['receipt','history']:[]);ok();
}
...
for(const changedAccount of [false,true]) {
 ...
 compile(find(clone,'continueAgreement')+';globalThis.run=continueAgreement;',scope);const task=scope.run();release(props.selected);await task;assert.deepEqual(writes,changedAccount?[]:['receipt','history']);ok();
}
