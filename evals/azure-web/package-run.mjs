...
try{
 ...
 test('context includes bootstrap and transitive publication helpers',()=>{for(const p of ['services/azure-web/server.mjs','services/azure-web/Dockerfile','services/azure-web/build-outcome.mjs','services/azure-web/build-outcome.d.mts','scripts/check-copy.mjs','scripts/roomsVocabAllowlist.mjs','scripts/copy-room-scope.mjs','evals/dbattery/prosody-baseline-log.json'])assert(context.files.some(f=>f.path===p),p);});
 ...
 test('context includes every static Vite input',()=>{for(const p of ['studio.html','room.html','studio-layout-fixture.html','creator-layout-fixture.html','room-layout-fixture.html','site/creators.html'])assert(context.files.some(f=>f.path===p),p);});
