...
try{
 ...
 server=createServer(async(req,res)=>{
  ...
  if(url.pathname.startsWith('/api/')){
   ...
   requests.push({path:url.pathname,op,method:req.method,body,auth:req.headers.authorization,referrer:req.headers.referer,query:Object.fromEntries(url.searchParams)});
   ...
   if(url.pathname==='/api/replica-text-rehearsal'){
    if(op==='readiness'){const value={readiness:readiness(rid)};if(scenario==='removed'){value.readiness.context_items=[];value.readiness.selected=null;value.readiness.can_ask=false;value.readiness.state='needs_input';value.readiness.blockers=[{code:'selected_source_unavailable',responsibility:'owner'}];}if(!url.searchParams.get('sheet_id')||!url.searchParams.get('context_item_id')){value.readiness.selected=null;value.readiness.can_ask=false;value.readiness.state='needs_input';value.readiness.blockers.push({code:'rehearsal_selection_required',responsibility:'owner'});}if(scenario==='late-readiness'){pending.push(()=>send(200,value));return;}return send(200,value);}
    if(op==='ask'){
 ...
 const testSource=()=>page.getByRole('button',{name:/^(Test this source|इस सामग्री से पूछें)$/});
 const backTest=()=>page.getByRole('button',{name:/^(Back to private test|निजी सवाल पर लौटें)$/});
 const editSources=async()=>{await page.getByRole('button',{name:'Add or edit source material'}).click();await page.getByText('pendulum-notes.txt',{exact:true}).waitFor();};
 const mutations=()=>requests.filter(r=>r.method==='POST'&&['/api/context-items','/api/teacher-sheet','/api/replica-text-rehearsal'].includes(r.path));
 for(const width of [390,1440])await check(`source-bound round trip ${width}: actual shell, keyboard, untouched question and fresh attestations`,async()=>{
  await page.setViewportSize({width,height:900});await page.emulateMedia({reducedMotion:width===390?'reduce':'no-preference'});await open('ready',true);
  if(width===1440)await page.evaluate(()=>{const u=new URL(location.href);u.searchParams.set('lang','en');history.replaceState(null,'',u);});
  await page.locator('#ptr-question').fill('Keep this unsent question');for(const box of await page.locator('.ptr-attestation input').all())await box.check();
  await page.getByRole('button',{name:'Add or edit source material'}).focus();await page.keyboard.press('Enter');await testSource().waitFor();assert.equal(await page.locator('.vx-shell').count(),1);
  await testSource().scrollIntoViewIfNeeded();await page.screenshot({path:join(artifact,`source-${width}.png`)});const rect=await testSource().boundingBox();assert(rect.height>=44);assert(rect.x>=0&&rect.x+rect.width<=width);assert.equal(await testSource().innerText(),width===390?'इस सामग्री से पूछें':'Test this source');
  await testSource().focus();await page.keyboard.press('Space');await page.getByText('Review source: pendulum-notes.txt').waitFor();assert.equal(await page.locator('#ptr-question').inputValue(),'Keep this unsent question');assert.equal(await page.locator('.ptr-fields select').nth(0).inputValue(),SHEET);assert.equal(await page.locator('.ptr-fields select').nth(1).inputValue(),ITEM);
  assert.equal(await page.locator('.ptr-attestation input:checked').count(),0);assert(await page.getByRole('button',{name:'Ask privately',exact:true}).isDisabled());assert.deepEqual(mutations(),[]);
  const read=requests.filter(r=>r.op==='readiness').at(-1);assert.equal(read.auth,'Bearer '+TOKEN);assert.equal(read.query.replica_id,RID);assert.equal(read.query.context_item_id,ITEM);assert.equal(read.query.sheet_id,SHEET);
  assert.equal(new URL(page.url()).searchParams.get('lang'),width===390?'hi':'en');assert.equal(new URL(page.url()).searchParams.get('replica'),RID);assert.equal(new URL(page.url()).searchParams.get('view'),'rehearsal');
  await page.locator('#ptr-question').scrollIntoViewIfNeeded();await page.screenshot({path:join(artifact,`returned-${width}.png`)});assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false);
  const stored=await page.evaluate(()=>Object.entries(localStorage).concat(Object.entries(sessionStorage)).map(([,value])=>value).join('\n'));assert(!stored.includes('Keep this unsent question'));assert(!page.url().includes('unsent'));
