...
function context(){const ctx={identity:'owner-a',accessToken:'token-a',selected:{replica_id:RID},rehearsalReturn:{current:null},reissueMounted:{current:true},reissueCurrent:{current:{identity:'owner-a',accessToken:'token-a',selected:{replica_id:RID}}},isPrivateTextId:id=>id===ITEM,calls:[]};ctx.chooseRoom=room=>ctx.calls.push(room);ctx.setEnrichView=view=>ctx.calls.push(view);runInNewContext(compile('globalThis.edit='+callbacks.onEditContext+';globalThis.test='+callbacks.onTestSource+';'),ctx);return ctx;}
...
check('actual owned row callback preserves draft and exact context handle',()=>{const c=context();c.edit(draft);c.calls=[];c.test({replicaId:RID,itemId:ITEM});assert.equal(c.rehearsalReturn.current.draft.question,draft.question);assert.equal(c.rehearsalReturn.current.draft.sheetId,OTHER);assert.equal(c.rehearsalReturn.current.draft.contextItemId,ITEM);assert.deepEqual(c.calls,['rehearsal']);});
