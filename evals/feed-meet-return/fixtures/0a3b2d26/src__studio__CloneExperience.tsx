      ...
      <main className="vx-main">
        <AnimatePresence mode="wait" initial={false}>
          {selected && room === "rehearsal" ? (
            <motion.div className="vx-scene" key={`rehearsal:${selected.replica_id}`} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><Suspense fallback={<p role="status">Opening private draft test</p>}><PrivateTextRehearsal token={accessToken} replicaId={selected.replica_id} lifecycle={selected.lifecycle} onBack={() => chooseRoom("enrich")} onEditContext={() => { setEnrichView("files"); chooseRoom("enrich"); }} onAuthError={onAuthError} /></Suspense></motion.div>
          ) : needsAgreement ? (
