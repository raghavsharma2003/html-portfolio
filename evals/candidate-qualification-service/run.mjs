...
function fixture(options={}){
 ...
 const candidate={candidate_id:CID,dataset_id:DATASET,replica_id:RID,owner_user_id:OWNER,status:'draft',run_state:options.collecting?'collecting':'complete',kind:'prompt_policy',target_layers:['delivery'],
  ...
  materialization_job_id:JOB,model_commitment:hash({protocol:'vyakti.private-text-materialization.v1',name:'azure-foundry-structured-output',version:`${AZURE_DIALOGUE_API_VERSION}:${DIALOGUE_PROMPT}`,model:'gpt-4.1-mini',base_model_commitment:ENV.AZURE_CORRECTION_BASE_MODEL_COMMITMENT,revision_binding:expectedBinding}),candidate_core_hash:hash(renderPrivateCorrectionCandidate(rt,artifact).core),baseline_hash:hash(compileReplicaRuntimeCore(runtime.profile_definition,runtime.calibration_definition)),total:testRows.length*2,
...
await test('artifact, manifest, source, missing identity and cross-response revision mutations refuse',async()=>{
 for(const mutate of [c=>c.artifact.fixture='changed',c=>c.build_manifest.source_set_hash='f'.repeat(64),c=>c.materialized_artifact='f'.repeat(64),c=>c.materialized_manifest='f'.repeat(64),c=>c.identities.pop(),c=>c.item_examples.pop(),c=>c.item_examples[0].feedback_id=train[0].feedback_id,c=>c.item_examples[0].role='candidate',c=>c.profile_version++,c=>c.calibration_version++,c=>c.base_capability_id=uid(333),c=>c.baseline_hash='f'.repeat(64),c=>c.candidate_core_hash='f'.repeat(64),c=>c.model_commitment='f'.repeat(64),c=>c.identities[0].system_fingerprint='fp_changed']){
