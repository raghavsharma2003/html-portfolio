...
console.log("\n── 4. the owner key reuses roomRelStateFromFollower/*Reset UNCHANGED; the stage-counts aggregate excludes the owner ──");
{
  ...
  const dbCounts = async (sql, params) => {
    ...
    if (sql.includes("select subject_person_id from vy_replica")) {
      return String(params[0]) === AGENT ? [{ subject_person_id: OWNER_PERSON }] : [];
    }
    if (sql.includes("select trust, rupture_open from vy_rel_state")) {
      const [personId, agentId] = params.map(String);
      return personId === OWNER_PERSON && agentId === AGENT ? [ownerDyadRow] : [];
    }
    throw new Error(`unexpected: ${sql}`);
  };
  ...
  const dbCountsNoExclusion = async (sql, params) => {
    ...
    if (sql.includes("select subject_person_id from vy_replica")) return [];
    throw new Error(`unexpected: ${sql}`);
  };
  ...
  const dbCountsAtFloor = async (sql, params) => {
    ...
    if (sql.includes("select subject_person_id from vy_replica")) return String(params[0]) === AGENT ? [{ subject_person_id: OWNER_PERSON }] : [];
    if (sql.includes("select trust, rupture_open from vy_rel_state")) return [{ trust: 0.8, rupture_open: false }];
    throw new Error(`unexpected: ${sql}`);
  };
