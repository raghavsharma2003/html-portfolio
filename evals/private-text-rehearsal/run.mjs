...
try{
  ...
  if(url.pathname.startsWith('/api/')){
   ...
   if(url.pathname==='/api/replica-text-rehearsal'){
    ...
    if(op==='ask'){
     ...
     assert.equal(body.statement_set,'private-text-rehearsal/v1');assert.deepEqual(body.attestations,{...Object.fromEntries(statements.map(s=>[s.id,true])),...(body.parent_request_id?{authorize_private_text_followup:true}:{})});assert.equal(body.expected_snapshot_hash,hash);assert.equal(body.sheet_id,SHEET);assert.equal(body.context_item_id,ITEM);
 ...
 await check('settled saved answer starts an explicit follow-up and survives reload',async()=>{
  await open();await fill();await page.getByRole('button',{name:'Ask privately',exact:true}).click();await page.locator('.ptr-answer').waitFor();
  const parentId=new URL(page.url()).searchParams.get('rehearsal_request');await page.getByRole('button',{name:'Ask a follow-up'}).click();
  await page.getByText(/saved question and answer will be used only as context/).waitFor();await page.locator('#ptr-question').fill('Why is it two seconds?');
  assert.equal(await page.locator('.ptr-attestation input').count(),0);await page.getByRole('button',{name:'Ask privately',exact:true}).click();await page.locator('.ptr-answer').waitFor();
  const asks=requests.filter(row=>row.path==='/api/replica-text-rehearsal'&&row.op==='ask');assert.equal(asks.length,2);assert.equal(asks[1].body.parent_request_id,parentId);assert.equal(asks[1].body.attestations.authorize_private_text_followup,true);
  const childId=new URL(page.url()).searchParams.get('rehearsal_request');assert.notEqual(childId,parentId);await page.reload();await page.locator('.ptr-answer').waitFor();assert.equal(askCount(),2);assert.equal(new URL(page.url()).searchParams.get('rehearsal_request'),childId);
 });
