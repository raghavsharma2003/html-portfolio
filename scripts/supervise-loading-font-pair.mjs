...
const runner=resolve(candidate,'scripts/run-loading-font-pair.mjs');
const runnerHash=createHash('sha256').update(await readFile(runner)).digest('hex');
const sourcePins={};for(const file of ['run-loading-font-pair.mjs','supervise-loading-font-pair.mjs','loading-font-collector.mjs'])sourcePins[file]=createHash('sha256').update(await readFile(resolve(candidate,'scripts',file))).digest('hex');
