...
import { ROOM_VOCAB_PATH, roomCopySectionSource } from "./copy-room-scope.mjs";
...
const SCOPES = [
  ...
  { dir: "src/creatorStudio/", rules: "full", codename: true },
...
const EXTRA_FILES = [
  { file: "studio.html", rules: "full", codename: false },
 ...
 * Studio naming restriction. copy-room-scope.mjs retains Room vocabulary on
 * recipient/distribution surfaces and named Room sections in mixed locale
 * tables. Every other copy rule still uses the complete scopes above. */
...
export async function scanScopedSource(rel, source, options = {}) {
  const hits = scanSource(rel, source, { ...options, roomsVocab: ROOM_VOCAB_PATH.test(rel) });
  const roomSections = await roomCopySectionSource(rel, source);
  if (roomSections !== null) {
    hits.push(...scanSource(rel, roomSections, { ...options, roomsVocab: true })
      .filter(hit => hit.rule === "rooms-vocabulary"));
  }
  return hits;
}
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  for (const scope of SCOPES) {
    for (const rel of walk(scope.dir)) {
      ...
      const found = (await scanScopedSource(rel, src, { rules: scope.rules, codename })).map((o) => ({ ...o, file: rel }));
      if (WAIVED.has(rel)) {
  ...
  for (const extra of EXTRA_FILES) {
    ...
    const found = (await scanScopedSource(rel, src, { rules: extra.rules, codename })).map((o) => ({ ...o, file: rel }));
    offences.push(...found);
  }
