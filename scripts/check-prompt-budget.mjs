// Guard the operational Room prompt boundary through the real compiler.
// Every fixture supplies an explicit Vyakti agent. There is no implicit
// product persona and no separate live-call prompt lane in this repository.
//
...
//   2. OPERATIONAL CAPS — what api/_azure-surface-reply.js slices the Room
//      prompt at today. Every real fixture is measured
...
//
// Also runs the compiler's shape-lint structural checks (T10 pinned last,
// appended-last-set-of-exactly-two, and the selected agent's crisis floor).
...
// ── guard/guarded parity — the Azure Room adapter's literal caps must equal the
...
const capOf = (src, name) => {
  ...
  if (!m) throw new Error(`could not find ${name} in api/_azure-surface-reply.js — guard is stale, fix it`);
  return Number(m[1].replace(/_/g, ""));
};
const roomProviderSrc = readFileSync(join(ROOT, "api/_azure-surface-reply.js"), "utf8");
const liveCoreCap = capOf(roomProviderSrc, "SYSTEM_MAX");
const liveTailCap = capOf(roomProviderSrc, "TAIL_MAX");
console.log("\n── guard/guarded parity (Azure Room adapter vs compiler manifest) ──");
if (liveCoreCap !== OPERATIONAL_CORE_CAP || liveTailCap !== OPERATIONAL_TAIL_CAP) {
  failed = true;
  console.log(
    `FAIL  Azure Room caps (core=${liveCoreCap}, tail=${liveTailCap}) != compiler OPERATIONAL caps ` +
      `(core=${OPERATIONAL_CORE_CAP}, tail=${OPERATIONAL_TAIL_CAP}) — guard and guarded have drifted`,
  );
} else {
  console.log(`  ok  Azure Room caps match compiler's declared OPERATIONAL caps exactly`);
}
...
for (const f of BUDGET_FIXTURES) {
  ...
  const crisisLines = f.input.agent?.CRISIS_LINES;
  if (!crisisLines || !core.includes(crisisLines)) {
    failed = true;
    console.log(`FAIL  [${f.id}] selected agent's crisis floor is missing from compiled core`);
  }
...
// (sentence-shaped English, first-person line-initial) on a clean vs.
...
if (failed) {
  console.error(
    "\nA hard gate failed — either the Azure Room operational prompt caps were exceeded, " +
      "the manifest's own arithmetic is broken, or a structural shape-lint assertion (T10 last, selected-agent crisis floor intact, " +
