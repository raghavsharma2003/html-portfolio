...
// Three cold contexts per revision and language. AB/BA/AB for English,
// BA/AB/BA for Hindi: six pairs with equal overall first-position exposure.
export const ORDER = Object.freeze([
 ['baseline','/studio'],['candidate','/studio'],['candidate','studio-hi'],['baseline','studio-hi'],
 ['candidate','/studio'],['baseline','/studio'],['baseline','studio-hi'],['candidate','studio-hi'],
 ['baseline','/studio'],['candidate','/studio'],['candidate','studio-hi'],['baseline','studio-hi'],
]);
...
assert.match(basename(roots.baseline),/^expert-loading47-baseline$/,'Use a private baseline isolate, never frozen45');
assert.match(basename(roots.candidate),/^expert-auth-loading47$/);
...
async function fingerprint(root) {
  ...
  return {assets,source:[...files],package:hash(await readFile(join(root,'package.json'))),lock:hash(await readFile(join(root,'package-lock.json'))),config:hash(await readFile(join(root,'api/_config.js')))};
}
const before={};
try {
  ...
  for(const revision of ['baseline','candidate']) {
    ...
    const generated=join(roots[revision],'scripts',`_loading47-measure-${process.pid}.mjs`);
  ...
  assert.deepEqual(changed,['src/studio/StudioApp.tsx','src/studio/auth-entry.css','src/studio/personalAuthLocale.tsx'].sort(),'Only reviewed product changes permitted');
  ...
  browser=await chromium.launch({...(executablePath?{executablePath}:{}),args:['--no-sandbox','--disable-background-networking']});
  await writeFile(join(out,'plan.json'),JSON.stringify({acceptance:false,order:ORDER,sourceFingerprints,before,browserVersion:browser.version(),browserExecutableHash:hash(await readFile(executablePath||chromium.executablePath())),budgets:modules.baseline.BUDGETS,throttle:modules.baseline.THROTTLE,viewport:modules.baseline.VIEWPORT},null,2));
  ...
  const groups=[];
  for(const revision of ['baseline','candidate'])for(const name of ['/studio','studio-hi']){const m=modules[revision],runs=rows.filter(r=>r.revision===revision&&r.target===name).map(r=>r.result);assert.equal(runs.length,3);const result=m.summarizeRuns(m.TARGETS.find(t=>t.name===name),runs);groups.push({revision,result,findings:m.evaluateBudgets(result)});}
