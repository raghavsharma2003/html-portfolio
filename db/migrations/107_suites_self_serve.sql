...
-- Added at the merge (2026-09-04): the two weekly funnel counts
-- (`suitesFunnelThisWeek`) read `vy_room.org_attached_at >= $1` and
-- `vy_org.created_at >= $1`; the live EXPLAIN seq-scanned both because
-- neither column had an index. Partial on the Room side (most Rooms sit in
-- no Suite), plain on the org side.
