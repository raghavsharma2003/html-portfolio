...
import {
  ...
  markChallengeUploaded,
  clientSource,
} from "./_replica-liveness.js";
import { getPendingSource, finalizeOwnedSource } from "./_replica-source.js";
import {
  ReplicaStorageError,
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (body.op === "finalize") {
      ...
      if (!source || source.state !== "quarantined") {
        return res.status(409).json({ error: "challenge_source_rejected" });
      }
      const challenge = await markChallengeUploaded(
        q, user.id, body.replica_id, body.challenge_id, body.source_id,
      );
      return challenge
        ? res.status(200).json({ challenge, source: clientSource(source), verification: "pending" })
        : res.status(409).json({ error: "challenge_expired_before_finalize" });
    }
