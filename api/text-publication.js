...
import {createTextPublicationVisitorHandler} from './_text-publication-runtime.js';

const run=createTextPublicationVisitorHandler({db:q,store,engine,resolveGenerator:createProductionDialogueGenerator,
 gateReply,hasGate,honestyContextFor,loadNeverRules,compileNeverRules,requireUser:async req=>{
  const user=await requireUser(req);
  if(!allow(user.id,'text_publication_visitor',40))throw Object.assign(new Error('slow_down'),{code:'slow_down',status:429});
  return user;
 }});
export default async function handler(req,res){
 ...
 if(!allow(ipOf(req),'text_publication_visitor_ip',90))return res.status(429).json({error:'slow_down'});
 return run(req,res);
}
