...
export function createVoiceAllocationAdmissionHandler({db,env=process.env,now=()=>Date.now(),consume=input=>consumeVoiceAllocationChild(db,input)}={}){return async(req,res)=>{
 ...
 try{
  ...
  return reply(200,await consume(input));
