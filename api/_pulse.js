 ...
 * on. Every literal column is wrapped in `min(...)` (the three uuids as
 * `min(($n)::text)::uuid`, because Postgres has `min(text)` and no
 * `min(uuid)`: the live EXPLAIN at the merge refused the plain form with
 * "function min(uuid) does not exist", which the fake db could not know);
 * the one real aggregate,
...
async function publishCombo(db, weekId, roomId, weekStart, labels) {
  const rows = await db(
    `insert into vy_room_pulse_combo (combo_id, week_id, room_id, week_start, labels, follower_count, computed_at)
     select min(($1)::text)::uuid, min(($2)::text)::uuid, min(($3)::text)::uuid, min(($4)::date), min(($5)::text[]), count(*)::int, min(now())
       from (
         select distinct o.person_id
           from vy_room_pulse_optin o
