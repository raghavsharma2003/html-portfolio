...
export default async function handler(req, res) {
  ...
  if (!allow(ipOf(req), "replica_voice_trial", 30)) return res.status(429).json({ error: "slow_down" });
  try {
    ...
    return res.status(status).json({ error: status === 500 ? "voice_trial_failed" : String(error.code || error.message) });
  }
}
