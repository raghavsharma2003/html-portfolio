...
import { selfCheckServing } from "./_self-check-serving.js";

// ═════════════════════════════════════════════════════════════════════════
// (a) env presence, by NAME only
// ═════════════════════════════════════════════════════════════════════════
//
// Legacy defaults mirror `scripts/write-config.mjs`'s two lists: `REQUIRED_ENV`
 ...
 *
 * Explicit Azure selection uses the provider classifier's effective names
 * and adds missing names once. Validation findings are fixed provider codes,
 * never values. The exported lists remain the legacy build-script mirror.
 * In the legacy default, every `MANIFEST_ONLY_ENV` row is `required: false` —
...
export function envPresence(env = process.env) {
  const serving = selfCheckServing(env);
  const required = new Set(serving?.required || REQUIRED_ENV);
  const out = [];
  const names = new Set([...REQUIRED_ENV, ...OPTIONAL_ENV, ...MANIFEST_ONLY_ENV, ...required]);
  for (const name of names) out.push({ name, required: required.has(name), present: Boolean(env[name]) });
  return out;
}
...
export async function runSelfCheck(deps = {}) {
  ...
  optionalAbsent.sort();
  const serving = selfCheckServing(env);
  if (serving) checks.push(serving.check);
