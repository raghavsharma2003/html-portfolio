...
function keyConfig(env = process.env) {
  const keyId = String(env.REPLICA_EVAL_KEK_ID || "").trim();
  const encoded = String(env.REPLICA_EVAL_KEK_B64 || "").trim();
  if (!KEY_ID.test(keyId)) fail("candidate_eval_encryption_key_id_required");
  let key;
  try { key = Buffer.from(encoded, "base64"); } catch { fail("candidate_eval_encryption_key_required"); }
  if (key.length !== 32 || key.toString("base64").replace(/=+$/, "") !== encoded.replace(/=+$/, ""))
    fail("candidate_eval_encryption_key_required");
  return { keyId, key };
}
...
function aadFor(binding) {
  ...
  if (values.some((value) => !value) || !SHA256.test(values.at(-1)))
    fail("candidate_eval_encryption_binding_invalid", 400);
  return Buffer.from(`vyakti.candidate-eval-asset.v1|${values.join("|")}`, "utf8");
}

export function evaluationTextHash(text) {
  return createHash("sha256").update(String(text), "utf8").digest("hex");
}

export function encryptEvaluationText(text, binding, env = process.env) {
  const plaintext = String(text || "");
  if (!plaintext) fail("candidate_eval_text_required", 400);
...
export function decryptEvaluationText(record, binding, env = process.env) {
  const { keyId, key } = keyConfig(env);
  if (record?.algorithm !== "AES-256-GCM" || record?.key_id !== keyId)
    fail("candidate_eval_encryption_key_unavailable");
  const aad = aadFor(binding);
  if (createHash("sha256").update(aad).digest("hex") !== record.aad_sha256)
    fail("candidate_eval_asset_binding_invalid", 409);
  try {
    ...
    if (dataKey.length !== 32) fail("candidate_eval_asset_decryption_failed", 409);
    ...
    fail("candidate_eval_asset_decryption_failed", 409);
  }
}
