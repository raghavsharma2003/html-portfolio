...
console.log("changes none of those five verdicts.");
console.log("");
console.log("The two anthropic rows are NOT candidates and must not be read as ones. Updated after [R1]");
console.log("(2026-08-18), which re-ran both in full at 192 calls each once the $20 OpenRouter key limit");
console.log("was raised — the pre-R1 version of this note described denominators of 14/14 and 8/9 that no");
console.log("longer exist:");
console.log("  * anthropic/claude-opus-4.8 is the model that WROTE the archived ground truth. Its row is a");
console.log("    complete 96-unit TEST-RETEST measurement of the ground truth's own reliability (74/96 =");
console.log("    77.1%), reported as DRAFT.md §5.11 and drawn as F1's ceiling band. It is the ceiling the");
console.log("    candidates are measured against, not a candidate. Clustering moves it by -1.0pp and");
console.log("    changes nothing.");
console.log("  * anthropic/claude-opus-5 is INVALID-RUN (parse): 128 of 192 replies came back empty because");
console.log("    reasoning consumed the 120-token cap, so its n=17 is a parse-selected denominator. Its");
console.log("    degenerate 100%-agree cluster bootstrap (every observed unit already agrees) is exactly the");
console.log("    noise that label warns about, not a new finding about clustering.");
